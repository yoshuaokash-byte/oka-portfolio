...
const ethPnl = p.ETH_THB ? ((p.ETH_THB - 94782) / 94782 * 100).toFixed(1) + '%' : const fetPnl = p.FET_THB ? ((p.FET_THB - 6.05) / 6.05 * 100).toFixed(1) + '%' : '?';
'?';
const prompt = `あなたは世界最高峰の投資アナリストです。以下のポートフォリオを分析し、今日のアクションプ
現在価格:
...
今日の日付: ${new Date().toLocaleDateString('ja-JP', {timeZone:'Asia/Bangkok', year:'numeric',
XRP・FET判断期限まで: ${Math.ceil((new Date('2026-05-15') - new Date()) / 86400000)}日
NVDA決算まで: ${Math.ceil((new Date('2026-05-20') - new Date()) / 86400000)}日
...
const STOCK_SANITY = { NVDA:[80,400], QQQ:[400,600], MSFT:[300,600] };
async function fetchStocks(errors=[]) {
try {
const key = process.env.TWELVE_DATA_API_KEY;
if (!key) throw new Error('No TWELVE_DATA_API_KEY');
const r = await fetch(
`https://api.twelvedata.com/price?symbol=NVDA,QQQ,MSFT&apikey=${key}`,
...
const [min, max] = STOCK_SANITY[sym];
return (v >= min && v <= max) ? v : FALLBACK_STOCKS[sym];
};
return {
NVDA: parse(d?.NVDA?.price, 'NVDA'),
QQQ: parse(d?.QQQ?.price, 'QQQ'),
MSFT: parse(d?.MSFT?.price, 'MSFT'),
};
} catch(e) {
...
// ─── ALERT RULES ───
const alerts = [];
if (p.NVDA >= 260) alerts.push(` NVDA $${p.NVDA} → $260超！1株利確
if (p.NVDA > 0 && p.NVDA <= 190) alerts.push(` NVDA $${p.NVDA} → $190割れ！即撤退
if (p.SOL_USD != null && p.SOL_USD >= 156) alerts.push(` SOL $${fmt2(p.SOL_USD)} → +50%超
if (p.ETH_USD != null && p.ETH_USD >= 3982)alerts.push(` ETH $${fmt2(p.ETH_USD)} → +50%超
if (p.FET_THB != null && p.FET_THB <= 3.63)alerts.push(` FET ฿${fmt2(p.FET_THB)} → -40%割
if (p.SOL_USD != null && p.SOL_USD >= 120) alerts.push(` SOL $${fmt2(p.SOL_USD)} → 急騰！停
// Send urgent alerts
